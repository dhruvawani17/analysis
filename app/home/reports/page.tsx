  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[20px] font-bold text-[#F0F0F5]" style={{ fontFamily: "'General Sans', system-ui, sans-serif" }}>Reports</h1>
          <p className="text-[13px] text-[#6B7280] mt-0.5">Auto-generated analysis reports</p>
        </div>
      </div>
      ...
      {!isLoading && datasets && datasets.length === 0 && (
        <div className="text-center py-20">
          <FileBarChart className="w-12 h-12 text-[#2a2d3e] mx-auto mb-4" />
          <p className="text-[15px] font-semibold text-[#F0F0F5] mb-1" style={{ fontFamily: "'General Sans', system-ui, sans-serif" }}>No reports yet</p>
          <p className="text-[13px] text-[#6B7280] mb-4">Upload a dataset to generate reports</p>
      ...
      {datasets && datasets.length > 0 && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {datasets.map((d) => (
            <Link key={d.id} href={`/datasets/${d.id}/report`}>
              <div className="group p-5 rounded-xl border border-[#1C1E2E] bg-[#12141F] hover:border-[#2a2d3e] hover:bg-[#161827] transition-all cursor-pointer">
                <div className="flex items-start justify-between mb-3">
                  <div className="w-10 h-10 rounded-xl bg-[#F59E0B]/15 flex items-center justify-center">
                    <FileBarChart className="w-5 h-5 text-[#F59E0B]" />
                  </div>
                ...
                <p className="text-[12px] text-[#6B7280]">View report →</p>
              </div>
            </Link>
          ))}
        </div>
