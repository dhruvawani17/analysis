  ...
  async function handleEmailSignIn(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "link") {
        ...
        setError(err.message?.includes("auth/invalid-credential") ? "Invalid email or password" : "Failed to sign in");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-[#050608] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <p className="text-center text-sm text-[#A1A1AA] mt-6" style={{ fontFamily: "'General Sans', system-ui, sans-serif" }}>
          ...
          <Link href="/sign-up" className="text-[#6366F1] hover:text-[#818CF8] font-medium transition-colors">Sign up</Link>
        </p>
      </div>
    </div>
  );
